...
function generateWorldVolume(world) {
  console.log('XXX generateWorldVolume:: geometry.world', world);
...
function processVolume(volume) {
  console.log('XXX processVolume:: add standard volume', volume);
...
function initializeAssemblies(assemblies, geometry) {
  ...
  geometry.volumes.forEach(volume => {
    ...
    if (!assemblies[volume._compoundId] && volume.type === 'assembly') {
      console.log('XXX processAssembly:: new assembly', volume);
...
export function generateJson(geometry){
  console.log('XXX generateJson:: geometry', geometry);
  ...
  console.log('XXX generateJson:: outputStructure', outputStructure);
...
export function convertToMultiplePlacements(geometry) {
  ...
  console.log('XXX convertToMultiplePlacements:: json', json);
  ...
  // Keep track of assemblies that are children of other assemblies
  ...
  geometry.volumes.forEach(volume => {
    if (volume.mother_volume && assemblies[volume.mother_volume]) {
      ...
      // If this child is itself an assembly, mark it as a nested assembly
      if (volume.type === 'assembly') {
        nestedAssemblies.add(volume.name);
        ...
        nestedAssemblyRelationships.push({
          ...
          parentAssembly: volume.mother_volume,
          position: volume.position || { x: 0, y: 0, z: 0 },
          rotation: volume.rotation || { x: 0, y: 0, z: 0 },
          childType: childType
        });
        
        console.log(`Stored nested assembly relationship: ${volume.name} (${childType}) inside ${volume.mother_volume}`);
      } else {
        // Only add non-assembly children to the parent assembly's children array
  ...
  Object.keys(assemblyTypes).forEach(assemblyTypeId => {
    ...
    // Filter out nested assemblies from the instances list
    const standaloneInstances = typeData.instances.filter(instance => !nestedAssemblies.has(instance.name));
    console.log(`Filtered ${typeData.instances.length - standaloneInstances.length} nested assemblies, leaving ${standaloneInstances.length} standalone instances`);
    
    // Find all nested assemblies that are of this type - do this check early
    const earlyNestedInstancesCheck = nestedAssemblyRelationships.filter(rel => {
      // Skip if we've already processed this nested assembly
      if (processedNestedAssemblies.has(rel.childAssembly)) {
        return false;
      }
      
      // Match by type as in the existing code
      const isMatch = (
    ...
    // Skip creating the template if there are only nested instances and no standalone instances
    if (standaloneInstances.length === 0 && earlyNestedInstancesCheck.length > 0) {
      console.log(`Skipping template creation for ${assemblyTypeId} as it only has nested instances`);
      
      // Mark all nested instances as processed to avoid duplicates
      earlyNestedInstancesCheck.forEach(rel => {
        processedNestedAssemblies.add(rel.childAssembly);
        console.log(`Marked ${rel.childAssembly} as processed to prevent duplicate definitions`);
      });
    ...
    console.log(`Number of instances to process: ${standaloneInstances.length} standalone + ${nestedAssemblyRelationships.length} nested`);
    
    // Add placements for standalone instances (not nested inside other assemblies)
    standaloneInstances.forEach((instance, index) => {
      // Log detailed information about each instance
      console.log(`Processing standalone instance ${index}: ${instance.name}`);
    ...
    nestedInstancesOfThisType.forEach(nestedRel => {
      ...
      const placement = {
        ...
        rotation: {
          x: nestedRel.rotation.x,
          y: nestedRel.rotation.y,
          z: nestedRel.rotation.z
        },
        // The parent is the parent assembly
        parent: nestedRel.parentAssembly,
        // Flag this as a nested assembly reference
  ...
  console.log('XXXXX convertedGeometry', convertedGeometry);
